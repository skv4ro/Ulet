...
public final class Verifier {
    private static final File PUBLIC_KEY_FILE = new File(System.getProperty("user.home") + "/desktop/public.key");
    ...
    public static void main(String[] args) {
        try {
            byte[] publicKeyBytes = FileUtils.readFileToByteArray(PUBLIC_KEY_FILE);
            X509EncodedKeySpec publicKeySpec = new X509EncodedKeySpec(publicKeyBytes);
            KeyFactory keyFactory = KeyFactory.getInstance(Generator.ALGORITHM);
            PublicKey publicKey = keyFactory.generatePublic(publicKeySpec);
            Signature sign = Signature.getInstance(Generator.SIGN_ALGORITHM);
